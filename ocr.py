...
class ocr_network(object):
    def __init__(self, birnn_type=2, lstm_num_layers=1, batch_size=32, max_timestep=None):
        ...
        self.seq_len = tf.placeholder(tf.int32)  # , shape=(self.batch_size)
        ...
        self.inputs_ = tf.placeholder(tf.float32, [self.batch_size, self.img_height, None, self.channels], name='inputs')  # Batch, Height, Width , Channel

        # self.learning_rate = tf.placeholder(tf.float32, name='learning_rate')    #0.001    #5e-3    #0.05    #
        # tf.summary.scalar('learning_rate', self.learning_rate)

        self.labels_ = tf.sparse_placeholder(tf.int32, name='labels')
        ...
        with tf.name_scope('lstm'):
            ...
            if self.birnn_type == 0:  # dynamic_rnn
                self.layer = tf.transpose(self.layer,
                                          [0, 2, 1, 3])  # [batch_size, feature_w, feature_h, cnn_out_channels]
                # `feature_w` is max_timestep in lstm.
                self.layer = tf.reshape(self.layer, [self.batch_size, -1,
                                                     feature_h * cnn_out_channels])  # feature_w(self.max_timestep) unknown
                print('lstm input shape: {}'.format(self.layer.get_shape().as_list()))

                # tf.nn.rnn_cell.RNNCell, tf.nn.rnn_cell.GRUCell
                self.cell1 = tf.nn.rnn_cell.LSTMCell(self.num_hidden, state_is_tuple=True)
                self.cell2 = tf.nn.rnn_cell.LSTMCell(self.num_hidden, state_is_tuple=True)

                self.stack = tf.nn.rnn_cell.MultiRNNCell([self.cell1, self.cell2], state_is_tuple=True)
                initial_state = self.stack.zero_state(self.batch_size, dtype=tf.float32)
                ...
                outputs = tf.reshape(outputs, [-1, self.num_hidden])  # [batch_size * max_timestep, self.num_hidden]
                ...
                self.layer = tf.reshape(self.layer, [self.batch_size, self.max_timestep,
                                                     feature_h * cnn_out_channels])  # feature_w(self.max_timestep) unknown

                print('lstm input shape: {}'.format(self.layer.get_shape().as_list()))

                self.forward_cell = [tf.nn.rnn_cell.LSTMCell(self.num_hidden) for _ in range(self.lstm_num_layers)]
                self.forward_stack = tf.nn.rnn_cell.MultiRNNCell(self.forward_cell, state_is_tuple=True)

                self.backward_cell = [tf.nn.rnn_cell.LSTMCell(self.num_hidden) for _ in range(self.lstm_num_layers)]
                self.backward_stack = tf.nn.rnn_cell.MultiRNNCell(self.backward_cell, state_is_tuple=True)

                # layer: [max_timestep, batch_size, feature_h * cnn_out_channels]
                layer = tf.unstack(self.layer, self.max_timestep, axis=1)

                # outputs: [max_timestep, batch_size, 2*num_hidden]
                outputs, _, _ = tf.nn.static_bidirectional_rnn(self.forward_stack, self.backward_stack, layer, dtype=tf.float32)
                # outputs: [max_timestep, batch_size, 2, num_hidden]
                outputs = [tf.reshape(t, [self.batch_size, 2, self.num_hidden]) for t in outputs]

                W1 = tf.Variable(tf.truncated_normal([2, self.num_hidden], stddev=np.sqrt(2.0 / (2 * self.num_hidden))))
                b1 = tf.Variable(tf.zeros([self.num_hidden]))
                W2 = tf.Variable(tf.truncated_normal([self.num_hidden, self.num_classes],
                                                                 stddev=np.sqrt(2.0 / self.num_hidden)))
                b2 = tf.Variable(tf.zeros([self.num_classes]))

                # [max_timestep, batch_size, num_hidden]
                outputs = [tf.reduce_sum(tf.multiply(t, W1), reduction_indices=1) + b1 for t in outputs]
                # Time major  [max_timestep, batch_size, num_classes]
                self.logits = [tf.matmul(t, W2) + b2 for t in outputs]
                ...
                outputs, _ = tf.nn.bidirectional_dynamic_rnn(self.forward_stack, self.backward_stack, self.layer,
                                                               dtype=tf.float32)

                outputs = tf.concat(outputs, axis=2)  # [batch_size, max_timestep, 2 * num_hidden]

                outputs = tf.reshape(outputs, [-1, 2 * self.num_hidden])  # [batch_size * max_timestep, 2 * num_hidden]
                ...
                outputs, _, _ = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(self.forward_cell, self.backward_cell,
                                                                               self.layer, dtype=tf.float32)
                ...
                # outputs: [max_timestep, batch_size, 2*num_hidden]
                outputs, _, _ = tf.contrib.rnn.stack_bidirectional_rnn(self.forward_cell, self.backward_cell, layer, dtype=tf.float32)
                # outputs: [max_timestep, batch_size, 2, num_hidden]
                outputs = [tf.reshape(t, [self.batch_size, 2, self.num_hidden]) for t in outputs]
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = tf.nn.ctc_loss(labels=self.labels_,
                                        inputs=self.logits,
                                        sequence_length=self.seq_len)
            ...
            self.label_error_rate = tf.reduce_mean(tf.edit_distance(tf.cast(self.decoded[0], tf.int32), self.labels_))
            tf.summary.scalar('label_error_rate', self.label_error_rate)

        self.summaries_op = tf.summary.merge_all()

        # Train Summaries
        self.train_writer = tf.summary.FileWriter(
            os.path.join(os.getcwd(), "ocrlogs/train"), self.sess.graph)

        # Test summaries
        self.test_writer = tf.summary.FileWriter(
            os.path.join(os.getcwd(), "ocrlogs/test"), self.sess.graph)
    ...
    def cnn_layer(self, layer, filter_size):
        layer = tf.layers.conv2d(layer, filter_size, 3, padding='SAME')  # filters 128(or 256)
    ...
    def train_step(self, batch_inputs, batch_labels, batch_seq_len, orig_labels):
        feed_dict = {
            self.inputs_: batch_inputs,
            self.labels_: batch_labels,
            self.seq_len: batch_seq_len
        }

        start_time = time.time()
        _, label_error_rate, loss, global_step, summary, dense_decoded = self.sess.run([self.optimizer, self.label_error_rate, self.loss, self.global_step, self.summaries_op, self.dense_decoded], feed_dict=feed_dict)
    ...
    def forward(self, img):
        ...
        feed_dict = {
            self.inputs_: im,
            self.seq_len: np.asarray([(img.shape[1]) // 16], dtype=np.int32)
        }

        dense_decoded = self.sess.run([self.dense_decoded], feed_dict=feed_dict)

        decoded_label = [self.decode_maps[j] for j in dense_decoded[0][0] if j != -1]
...
def train():
    ...
    try:
        while True:
            ...
            model.train_step(batch_inputs, batch_labels, batch_seq_len, orig_labels)
